...
def dep_head_required(
    function=None,
    redirect_to="/",
):
    """
    Decorator for views that checks that the logged-in user is a department head,
    redirects to the specified URL if necessary.
    """

    # Define the test function: checks if the user is active and a dep head
    def test_func(user):
        return user.is_active and user.is_dep_head or user.is_superuser
