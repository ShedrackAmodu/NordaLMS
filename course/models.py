...
def log_program_save(sender, instance, created, **kwargs):
    ...
    ActivityLog.objects.create(
        message=_(f"Program '{instance.title}' has been {verb}.")
    )
...
def log_program_delete(sender, instance, **kwargs):
    ActivityLog.objects.create(
        message=_(f"Program '{instance.title}' has been deleted.")
    )
...
def course_pre_save_receiver(sender, instance, **kwargs):
    """Ensure that each course has a unique slug before saving."""
...
def log_course_save(sender, instance, created, **kwargs):
    verb = "created" if created else "updated"
    ActivityLog.objects.create(message=_(f"Course '{instance.title}' has been {verb}."))


@receiver(post_delete, sender=Course)
def log_course_delete(sender, instance, **kwargs):
    ActivityLog.objects.create(
        message=_(f"Course '{instance.title}' has been deleted.")
    )
...
class LiveClass(models.Model):
    ...
    meeting_code = models.CharField(max_length=20, unique=True, blank=True)
    created_at = models.DateTimeField(auto_now_add=True)

    def save(self, *args, **kwargs):
        if not self.meeting_code:
            self.meeting_code = get_random_string(12)  # Generate a unique code
